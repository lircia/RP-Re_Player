...
运行 `一键启动.cmd` 会先构建项目，同时开放 HTTP `80` 与 HTTPS `443` 端口。默认 `IP=127.0.0.1` 时显示 `soers.de5.net`。已配置的回环记录按尾号映射：`127.0.0.N` 对应 `2N.soers.de5.net`，`172.0.0.N` 对应 `7N.soers.de5.net`，其中 `N=0-9`；`127.0.0.1` 特例使用根域名。其他 IP 直接显示实际地址。Wrangler 的本地 HTTPS 证书可能需要在浏览器中手动确认信任。
